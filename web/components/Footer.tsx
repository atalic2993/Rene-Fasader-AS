...
/** A framed row on a phone, a plain link from sm up. */
function ContactRow({
  href,
  label,
  note,
  ...
  return (
    <li>
      <a
        href={href}
        className="flex min-h-11 items-center gap-3 border border-white/15 px-4 py-3 text-sm transition-colors hover:border-sand/60 hover:text-sand sm:min-h-0 sm:border-0 sm:px-0 sm:py-0 sm:hover:border-0"
      >
        {icon}
        <span>{label}</span>
        {note ? <span className="ml-auto text-white/55 sm:ml-2">{note}</span> : null}
      </a>
    </li>
  );
}
...
export default function Footer() {
  return (
    <footer className="border-t border-petrol-line bg-petrol text-white">
      <div className="shell grid gap-6 py-8 sm:gap-8 sm:py-12 lg:grid-cols-12 lg:gap-12">
        <div className="lg:col-span-7">
          ...
          <p className="mt-3 max-w-[24rem] text-sm text-white/60 sm:mt-2">
            {footerCopy.tagline}
          </p>
        </div>

        <div className="lg:col-span-5">
          <h2 className="eyebrow text-white/60">Kontakt</h2>
      ...
      <div className="shell flex flex-wrap items-center justify-between gap-x-6 gap-y-2 border-t border-white/10 py-4 text-xs text-white/60 sm:py-5">
        <p>
